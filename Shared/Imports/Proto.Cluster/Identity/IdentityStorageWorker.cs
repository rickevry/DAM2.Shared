    ...
    class IdentityStorageWorker : IActor
    {
        ...
        private async Task<(PID?, SpawnLock?)> SpawnActivationAsync(Member activator, SpawnLock spawnLock, CancellationToken ct)
        {
            ...
            catch (TimeoutException)
            {
                ...
                _logger.LogInformation("[SpawnActivationAsync] Removing lock {@SpawnLock}", spawnLock);
                try
                {
                    await _storage.RemoveLock(spawnLock, CancellationTokens.WithTimeout(TimeSpan.FromSeconds(5d)));
                }
                catch (Exception e)
                {
                    _logger.LogError(e, "Failed to remove lock ");
                }
